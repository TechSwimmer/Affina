...
const Profile = () => {
    ...
    // Get functions and state related to posts of the logged-in user
    const {userPosts,deletePost, fetchPostsOfLoginUser, setUserPosts} = useContext(PostContext) 
 
    // Fetch the user's own posts once when the component mounts
    useEffect(() => {
        fetchPostsOfLoginUser();
        setUserPosts([])  // Clear previous posts initially 
    },[])

    return (
        <div className="p-6 min-h-[87vh] border-l mt-20  fixed rounded-md bg-gradient-to-b from-[#13072e] to-[#3f2182] border-gray-700 hidden md:block ml-20">
            {/* Profile card */}
